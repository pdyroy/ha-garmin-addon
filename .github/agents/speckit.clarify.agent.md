...
2. Load the current spec file and `.specify/memory/constitution.md`. Perform a structured ambiguity & coverage scan using this taxonomy. For each category, mark status: Clear / Partial / Missing. Produce an internal coverage map used for prioritization (do not output raw map unless no questions will be asked).
   ...
   **Integration & External Dependencies:**
   - External services/APIs and failure modes (Garmin Connect API, Ollama/OpenAI)
   - Data import/export formats (FIT files, TCX, GPX)
   - Protocol/versioning assumptions

   **Edge Cases & Failure Handling:**
   - Negative scenarios
   - Rate limiting / throttling (Garmin API limits)
   - Conflict resolution (e.g., concurrent syncs)

   **Constraints & Tradeoffs:**
   - Technical constraints (engine purity, zero external deps)
   - Explicit tradeoffs or rejected alternatives

   **Terminology & Consistency:**
   - Canonical glossary terms (VDOT, HR zones, training load, etc.)
...
4. Sequential questioning loop (interactive):
   ...
   - For multiple-choice questions:
     - **Analyze all options** and determine the **most suitable option** based on:
       - Best practices for sport science / fitness coaching applications
       - Common patterns in similar implementations
       - Risk reduction (security, performance, maintainability)
       - Alignment with the project constitution
