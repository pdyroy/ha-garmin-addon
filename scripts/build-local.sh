...
# Local build and test script for Pacer HA addon
#
# Usage:
...
rm -rf "${ADDON_DIR}/pacer/ha-garmin-fitness-coach-app"
rsync -a --exclude=node_modules --exclude=.next --exclude=.git \
    "${APP_SOURCE}/" "${ADDON_DIR}/pacer/ha-garmin-fitness-coach-app/"
ok "App source copied."

# ─── Step 2: Build the Docker image ──────────────────────────────────────────
BUILD_FROM=$(python3 -c "import json; print(json.load(open('${ADDON_DIR}/pacer/build.json'))['build_from']['${ARCH}'])")
...
if [ "${1:-}" = "--run" ]; then
    ...
    docker run --rm -it \
        ...
        -e "GARMIN_EMAIL=" \
        -e "GARMIN_PASSWORD=" \
        -e "DATABASE_URL=file:/data/pacer.db" \
