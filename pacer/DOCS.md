# Pacer Home Assistant Addon

## Overview

Pacer is an AI-powered sport scientist that analyzes your Garmin health
...
```text
Home Assistant OS
├── Pacer Addon (s6-overlay services)
...
1. **Start** the addon and open the **Web UI** (sidebar → Pacer).
...
1. **Linked Google Calendar (recommended)** — one-time setup:
   ...
   3. Open **Stress Board** and use **Connect Google Calendar**: paste the
      ...
      **Adding a second calendar:** the picker lists every calendar visible
      ...
      > `/share/pacer/` — the next **Stress Board run** adopts it into
...
2. **Events file** — export an `.ics`, convert with
   ...
   and drop `calendar_events.json` in `/share/pacer/`.
3. **HA-logged interactions** — log out-of-calendar contacts (coffee
   chats, family visits) to `/share/pacer/interactions.jsonl` via an
...
`/share/pacer/meeting_stress.json` plus `meeting_scores.csv` and
...
- **Stress Board / Google Calendar**: the OAuth token is read-only
  ...
  computed locally and written to `/share/pacer/` — they contain
