...
"""Garmin Connect authentication microservice for Pacer HA Addon.
...
def trigger_recompute() -> tuple[Response, int] | Response:
    ...
    try:
        ...
        env["DATABASE_URL"] = os.environ.get(
            "DATABASE_URL",
            "postgresql://postgres@127.0.0.1:5432/pacer",
        )
        subprocess.Popen(
...
def trigger_meeting_stress() -> tuple[Response, int] | Response:
    ...
    calendar_events.json dropped in /share/pacer/. Results are written
    back to /share/pacer/.
    """
    import subprocess
    import time

    events_file = "/share/pacer/calendar_events.json"
    gcal_linked = os.path.exists("/data/gcal-token.json") or os.path.exists(
        "/share/pacer/gcal-token.json"
    )
    ...
    try:
        ...
        return jsonify(
            ...
            results="/share/pacer/meeting_stress.json",
        )
    except Exception as exc:
        try:
            with open(status_file, "w") as f:
...
def meeting_stress_status() -> Response:
    ...
    out: dict = {
        ...
        or os.path.exists("/share/pacer/gcal-token.json"),
        "events_file": os.path.exists("/share/pacer/calendar_events.json"),
    }
    status_file = os.path.join(TOKEN_DIR, ".meeting_stress_status")
    try:
        if os.path.exists(status_file):
