...
# Pacer health check & crash tracker
...
LOG_DIR="${HOME}/.local/share/pacer"
CRASH_LOG="${LOG_DIR}/crash-history.csv"
PORT="${PORT:-${PACER_PORT:-${PACER_PORT:-3000}}}"
mkdir -p "${LOG_DIR}"
...
echo -e "${CYN}═══ Pacer System Health Check ═══${RST}"
echo ""
...
# 5. Pacer specific checks
echo ""
echo -e "${CYN}▸ Pacer Services${RST}"

# Postgres
if docker compose exec -T postgres pg_isready -U dev -d pacer &>/dev/null; then
    row_count=$(docker compose exec -T postgres psql -U dev -d pacer -tAc \
